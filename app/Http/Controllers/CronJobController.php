...
class CronJobController extends Controller
{
  ...
  public function checkPayment()
  {
    ...
    $this->checkPendingFeatures();
  }

  protected function checkPendingFeatures()
  {
    $iyzico_pending_features = FeaturedProperty::where([['payment_status', 'pending'], ['gateway_type', 'iyzico']])->get();

    foreach ($iyzico_pending_features as $iyzico_pending_feature) {
      if (!is_null($iyzico_pending_feature->conversation_id)) {
        $result = $this->IyzicoPaymentStatus($iyzico_pending_feature->conversation_id);
        if ($result == 'success') {
          $this->updateIyzicoPendingFeature($iyzico_pending_feature->id, 'complete');
        } else {
          $this->updateIyzicoPendingFeature($iyzico_pending_feature->id, 'reject');
        }
      }
    }
    ...
    try {
      ...
      $nowPaymentFeatures = FeaturedProperty::where([
        'gateway_type' => 'now_payment',
        'payment_status' => 'pending',
      ])->get();

      foreach ($nowPaymentFeatures as $nowPaymentFeature) {

        $paymentId = (int) $nowPaymentFeature->transaction_id;
        ...
        if (array_key_exists('payment_status', $responseData) && $responseData['payment_status'] == 'waiting') {
          ...
          $nowPaymentFeature->update([
            'payment_status' => 'complete',
          ]);

          $checkout = new PaymentController();
          $checkout->mailToAdminForFeaturedRequest($nowPaymentFeature);
        } else {
          // update payment status
          $nowPaymentFeature->update([
  ...
  //update pending memberships if payment is successfull
  private function updateIyzicoPendingFeature($id, $status)
  {
    $feature = FeaturedProperty::query()->findOrFail($id);
    $feature->update(['payment_status' => $status]);

    if ($status == 'complete') {
      $checkout = new PaymentController();
      $checkout->mailToAdminForFeaturedRequest($feature);
    }
  }
